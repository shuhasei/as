...
function moveLocal(dt){
  ...
  const rawInputLen=Math.hypot(screenRight,screenForward);

  // 一人称視点は、見下ろし視点とは完全に別の「カメラ基準」で計算する。
  // このカメラは yaw=0 で +Z を向くため、画面右はワールド座標の -X になる。
  // そのため D/→ の旋回角はマイナス、A/← の旋回角はプラスにする。
  if(renderMode==='3d'&&cameraMode===2){
    if(rawInputLen>.04){
      const horizontal=Math.abs(screenRight)>.08?Math.sign(screenRight):0;
      const vertical=Math.abs(screenForward)>.08?Math.sign(screenForward):0;
      const signature=`${horizontal},${vertical}`;
      if(signature!==firstPersonInputSignature){
        firstPersonInputBaseYaw=firstPersonTargetYaw;
        firstPersonInputSignature=signature;
      }
      const cameraRelativeTurn=-Math.atan2(screenRight,screenForward);
      firstPersonTargetYaw=firstPersonInputBaseYaw+cameraRelativeTurn;
      inputX=Math.sin(firstPersonTargetYaw)*rawInputLen;
      inputZ=Math.cos(firstPersonTargetYaw)*rawInputLen;
    }else{
      inputX=0;inputZ=0;
      firstPersonInputSignature='';
      firstPersonInputBaseYaw=firstPersonTargetYaw;
    }
    firstPersonYaw=dampAngle(firstPersonYaw,firstPersonTargetYaw,18,dt);
  }
...
function setupJoystick(){
  ...
  const applyPoint=(clientX,clientY)=>{
    ...
    const amount=Math.min(maxDistance,distance);
    const angle=Math.atan2(y,x);
    joy={x:Math.cos(angle)*amount/maxDistance,y:Math.sin(angle)*amount/maxDistance};
    const visualDistance=Math.min(40,maxDistance*.82);
    ui.stick.style.transform=`translate(${joy.x*visualDistance}px,${joy.y*visualDistance}px)`;
  };
  const pointerDown=e=>{
...
(function installHudLayoutFix(){
  ...
  style.textContent=`
    ...
    @media (pointer:coarse) and (orientation:landscape) and (max-height:560px){
      ...
      #joystick{left:12px !important;bottom:78px !important;transform:scale(.72);transform-origin:bottom left}
