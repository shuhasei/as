...
function moveLocal(dt){
  ...
  // 一人称では、入力した方向を新しい「正面」にする。
  // D/→なら右へ90度、A/←なら左へ90度向き、カメラも同じ方向を向く。
  // 入力を押し続けても毎フレーム回転し続けないよう、入力方向が変わった瞬間だけ
  // 現在の正面を基準に次の向きを決める。
  if(renderMode==='3d'&&cameraMode===2){
    if(rawInputLen>.04){
      ...
      const localTurnAngle=Math.atan2(screenRight,screenForward);
      firstPersonTargetYaw=firstPersonInputBaseYaw+localTurnAngle;
      // 正面が+Zのとき、右は+X、左は-Xになる。
