...
function firebaseCpuPrompt(request){
  ...
  const deliveryHints=[
    '今日は少しおしゃべりな気分',
    '思いついたことをそのまま話す',
    '直前の言葉を拾って、さらっと軽くツッコむ',
    '真面目に答えたあと、短いセルフツッコミを添える',
    '日常の小さな違和感を少しだけ大げさに言う',
    '前に出た話題を意外なところで自然に回収する',
    '相手へ興味を持って話題を広げる',
    'ゲームと関係ない雑談へ自然に寄り道する'
  ];
...
function connect(room,name){
  ...
  ws.onclose=event=>{if(socket!==ws)return;socket=null;stopCpuMeetingSpeech();stopMeetingVoice();stopGroupVoice(false);hangUpCall(false);clearIncomingCall(false);const detail=event.reason?`：${event.reason}`:'';if(joinPending)failJoin(`ルームへ接続できませんでした（code ${event.code}${detail}）。`,false);else showNotice(`接続が切れました（code: ${event.code}${detail}）。再読み込みしてください。`)};
...
function handle(m){
  if(m.type==='hello'){
    ...
    state=m.state;syncCargoCarryState();updateCpuSpeechButton();
    ...
    for(const move of Array.isArray(m.moves)?m.moves:[]){
      ...
      const point=new THREE.Vector3(Number(move.x)||0,0,Number(move.z)||0);
      const queue=model.userData.botMoveQueue||(model.userData.botMoveQueue=[]);
      const anchor=queue[queue.length-1]||model.userData.target;
      if(anchor.distanceToSquared(point)>.0004){
        if(model.position.distanceToSquared(model.userData.target)<.01&&!queue.length)model.userData.target.copy(point);
        else queue.push(point);
      }
      if(queue.length>12){
        const latest=queue[queue.length-1];
        model.position.copy(latest);model.userData.target.copy(latest);queue.length=0;
      }
  ...
  else if(m.type==='meetingEnded'){stopCpuMeetingSpeech();stopMeetingVoice();closeDialog('meetingDialog');resumeGroupVoiceCapture();updateGroupVoiceUi();showNotice(m.ejected?`${m.ejected.name}が追放されました。役職は公開されません。`:'誰も追放されませんでした')}
  ...
  else if(m.type==='gameFinished'){stopCpuMeetingSpeech();stopMeetingVoice();hangUpCall(true);saveResult(m.winner);openResult(m.winner)}
...
function makeFallbackModel(player){
  return {position:new THREE.Vector3(player.x||0,0,player.z||0),rotation:{y:player.rotation||0},visible:true,userData:{target:new THREE.Vector3(player.x||0,0,player.z||0),botMoveQueue:[],rotation:player.rotation||0,hidden:Boolean(player.hidden)}};
}
function draw2DMap(){
  if(!canvas2d)return;
...
function syncModels(){
  ...
  for(const p of state.players){
    ...
    }else if((m.position.x-p.x)**2+(m.position.z-p.z)**2>36){
      // 極端に遅れた場合だけ安全な最新位置へ瞬時に復帰し、壁を横切る補間は行わない。
      m.position.set(p.x,0,p.z);m.userData.target.copy(m.position);
      if(Array.isArray(m.userData.botMoveQueue))m.userData.botMoveQueue.length=0;
    }
...
function updateUI(){
  ...
  if(p&&COLORS[p.color]){
    ...
    if(colorSelect&&colorSelect.value!==p.color)colorSelect.value=p.color;
    localStorage.setItem('hiddenCrewSelectedColor',p.color);
  }
  if(p&&HAT_TYPES.has(p.hat||'none')){
    const hatSelect=$('hatSelect');
    if(hatSelect&&hatSelect.value!==(p.hat||'none'))hatSelect.value=p.hat||'none';
    localStorage.setItem('hiddenCrewSelectedHat',p.hat||'none');
  }
  const roleNames={impostor:'人狼',crew:'クルー',doctor:'医者',detective:'探偵',guard:'警備員',spectator:'観戦者'};
  ui.room.textContent=state.room;
  ui.status.textContent={lobby:'ロビー',playing:'プレイ中',meeting:'会議中',finished:'終了'}[state.phase]||state.phase;
  ui.role.textContent=`役職：${roleNames[p?.role]||'---'}`;
  const visiblePlayers=state.players.filter(player=>!player.practiceTarget);
  const cpuCount=visiblePlayers.filter(player=>player.bot).length;
  ui.playerCount.textContent=`${visiblePlayers.length}/12（CPU ${cpuCount}）`;
  ...
  const lobbyHost=host&&state.phase==='lobby';
  ui.start.classList.toggle('hidden',!lobbyHost);
  ui.settings.classList.toggle('hidden',!lobbyHost);
  ui.cpuControls?.classList.toggle('hidden',!lobbyHost);
  ui.cpuHelp?.classList.toggle('hidden',!lobbyHost);
  if(ui.cpuHelp)ui.cpuHelp.textContent=`1人で開始するとCPUが5人参加します。${firebaseAiStatusLabel()}`;
  if(ui.addCpu)ui.addCpu.disabled=visiblePlayers.length>=12;
  if(ui.removeCpu)ui.removeCpu.disabled=cpuCount===0;
  ui.actionBar.classList.toggle('hidden',state.phase!=='playing'||!p?.alive);
  ui.taskPanel.classList.toggle('hidden',state.phase!=='playing'||!p||!p.alive);
  ui.kill.classList.toggle('hidden',state.phase!=='playing'||p?.role!=='impostor');
  ui.kill.disabled=p?.role!=='impostor'||!p?.alive||!canKill()||!nearest.player;
  ui.kill.title=p?.role==='impostor'?'近くのクルーを攻撃（Q / Space）':'攻撃は人狼だけが使えます';
  ui.sabotage.classList.toggle('hidden',p?.role!=='impostor'||!p?.alive);
  ui.joystick.classList.toggle('hidden',state.phase!=='playing'||(!p?.alive&&!!spectatorTargetId));
  if(p){
    const done=p.tasksDone||0,total=p.taskTotal||0;
    ui.taskCounter.textContent=`${done}/${total}`;
    ui.taskProgress.style.width=`${total?done/total*100:0}%`;
    ui.tasks.innerHTML=p.role!=='impostor'&&!p.spectator
      ?(p.tasks||[]).map(task=>`<div class="task-row ${(p.completedTasks||[]).includes(task)?'done':''}"><span>${taskDisplayName(task)}</span><b>${(p.completedTasks||[]).includes(task)?'✓':'○'}</b></div>`).join('')
      :'<p>偽タスクを装いましょう。</p>';
  }
  updateSabotage();
  updateDoorLockdownVisuals();
  updateSpectatorUI();
  queueHudLayout();
}
...
function animate(now=performance.now()){
  ...
  try{
    ...
    const blockingDialog=document.querySelector('dialog[open]:not(#securityDialog)');
    if(blockingDialog){
      if(now-lastHudUpdate>=1000){lastHudUpdate=now;updateCooldown();updateSabotage()}
      animationErrorShown=false;return;
    }
    if(state?.phase==='playing'&&localModel&&!securityOpen)moveLocal(dt);
    for(const m of models.values()){
      if(m!==localModel&&m?.position&&m?.userData?.target){
        if(m.userData.isBot){
          // サーバーから届いた経路点を順番に通り、遅延時にも曲がり角を飛ばして壁を横切らない。
          const queue=m.userData.botMoveQueue||(m.userData.botMoveQueue=[]);
          if(m.position.distanceToSquared(m.userData.target)<.01){
            m.position.copy(m.userData.target);
            if(queue.length)m.userData.target.copy(queue.shift());
          }
          const dx=m.userData.target.x-m.position.x,dz=m.userData.target.z-m.position.z;
          const distance=Math.hypot(dx,dz);
          if(distance>.001){
            const step=Math.min(distance,3.25*dt);
            const nextX=m.position.x+dx/distance*step;
            const nextZ=m.position.z+dz/distance*step;
            if(!collidesWithMap(nextX,nextZ,.56,false)){m.position.x=nextX;m.position.z=nextZ}
            else{
              // 古い不正な座標を引きずらず、次のサーバー経路点で再開する。
              m.userData.target.copy(m.position);queue.length=0;
            }
          }
        }else{
    ...
    if(renderMode==='3d'&&renderer&&scene&&camera){
      if(!securityOpen){
        ...
        if(corpseModels.size&&now-lastCorpseUpdate>=125){lastCorpseUpdate=now;updateCorpseVisuals(now)}
...
function collidesWithMap(x,z,r=.62,includeEmergencyButton=true){
  ...
  if(includeEmergencyButton&&Math.hypot(x-EMERGENCY_BUTTON.x,z-EMERGENCY_BUTTON.z)<2.05+r)return true;
  return false;
}
...
function renderSecurityFeed(){
  const now=performance.now();const effectiveSecurityFps=securityTaskActive?(LOW_POWER_DEVICE?5:8):PERF.securityFps;if(now-securityLastRender<1000/effectiveSecurityFps)return;securityLastRender=now;
  updateSecurityCameraUi();
...
function openMeeting(reason){
  if(activeCallPeer)hangUpCall(true);
  stopCpuMeetingSpeech();
  pauseGroupVoiceCapture();stopMeetingVoice();updateGroupVoiceUi();$('meetingReason').textContent=reason;
  renderVotes();openDialog('meetingDialog');updateMeetingParticipationUi();updateCpuSpeechButton();
  if(canParticipateInMeeting()){
    meetingAutoVoicePending=true;
    setVoiceStatus('会議音声へ自動接続しています…');updateMeetingVoiceUi();
    setTimeout(()=>autoStartMeetingVoice(),80);
...
function renderVotes(){
  ...
  state.players.filter(p=>p.alive&&p.meetingEligible!==false&&!p.practiceTarget).forEach(p=>{const b=document.createElement('button');b.textContent=`${p.bot?'🤖 ':''}${p.name}`;b.onclick=()=>{send('vote',{targetId:p.id});disableVotes()};root.append(b)});
...
async function speakNextCpuMeetingLine(){
  ...
  try{
    const pcm=decodeGeminiPcmChunks([item.data]);
    if(!pcm.length)throw new Error('Geminiから音声が返りませんでした');
    await playGeminiPcm(pcm,item.sampleRate,generation);
    ...
    if(generation===cpuMeetingSpeechGeneration){
      ...
      setTimeout(speakNextCpuMeetingLine,item.scope==='call'?8:240);
    }
  }
}
function queueCpuMeetingAudio(message){
  ...
  cpuMeetingSpeechQueue.push(item);
  const queueLimit=scope==='call'?32:5;
  if(cpuMeetingSpeechQueue.length>queueLimit)cpuMeetingSpeechQueue.splice(0,cpuMeetingSpeechQueue.length-queueLimit);
...
async function stopCpuLiveCall(){
  const session=cpuLiveSession;cpuLiveSession=null;cpuLiveStarting=false;
  if(session&&!session.isClosed)try{await session.close()}catch{}
}
async function receiveCpuLiveAudio(session,target){
  try{
    for await(const message of session.receive()){
      if(cpuLiveSession!==session||activeCallPeer!==target.id)break;
      if(message?.type!=='serverContent')continue;
      const serverContent=message.serverContent||message;
      for(const part of serverContent.modelTurn?.parts||[]){
        const audio=part?.inlineData;
        if(!audio?.data||!String(audio.mimeType||audio.mime_type||'audio/pcm').startsWith('audio/'))continue;
        const rateMatch=String(audio.mimeType||audio.mime_type||'').match(/rate=(\d+)/i);
        queueCpuMeetingAudio({scope:'call',fromId:target.id,from:target.name,sampleRate:rateMatch?Number(rateMatch[1]):24000,data:audio.data});
      }
      const transcript=String(serverContent.outputTranscription?.text||'').trim();
      if(transcript)setVoiceStatus(`🤖 ${target.name}：${transcript}`,true);
    }
  }catch(error){
    if(cpuLiveSession!==session)return;
    console.error('Gemini Live audio receive failed',error);
    const detail=compactFirebaseError(error?.message||error);
    setVoiceStatus(`CPU音声の受信に失敗しました：${detail}`);
    showNotice(`CPU音声受信失敗：${detail}`);
  }
}
async function startCpuLiveCall(target){
  if(!target?.bot||cpuLiveStarting)return false;
  cpuLiveStarting=true;setVoiceStatus(`🤖 ${target.name}とのGemini音声通話へ接続中…`,true);
  try{
    await firebaseAiInitialization;
    if(!firebaseAiService)throw new Error(firebaseAiInitError||'Firebase AI is not ready');
    const liveModel=getLiveGenerativeModel(firebaseAiService,{
      model:'gemini-2.5-flash-native-audio-preview-12-2025',
      generationConfig:{
        responseModalities:[ResponseModality.AUDIO],
        speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName:clientGeminiVoiceFor(target.name)}}}
      },
      systemInstruction:[
        `あなたは人狼ゲーム中のCPUプレイヤー「${target.name}」です。`,
        '友達との個人通話として、自然な日本語で短く会話してください。',
        '毎回同じ返事をせず、質問へ直接答え、ときどき自分から質問してください。',
        'ゲーム内で確認できない目撃情報や犯人を作らず、自分の正体は明かさないでください。'
      ].join('\n')
    });
    const session=await liveModel.connect();cpuLiveSession=session;
    await session.connectionPromise;
    stopCpuMeetingSpeech();cpuMeetingSpeechEnabled=true;localStorage.setItem('hiddenCrewCpuSpeech','on');updateCpuSpeechButton();
    receiveCpuLiveAudio(session,target);
    startCpuCallRecognition();
    await session.send(`${target.name}として、まず「もしもし、聞こえてる？」のように短く挨拶してください。`,true);
    setVoiceStatus(`🤖 ${target.name}とGemini音声通話中。普通に話しかけてください。`,true);
    const chatInput=$('globalChatInput');if(chatInput)chatInput.placeholder=`${target.name}へ話す内容を入力`;
    return true;
  }catch(error){
    console.error('Gemini Live CPU call failed',error);await stopCpuLiveCall();
    setVoiceStatus(`CPU音声通話に接続できません：${compactFirebaseError(error?.message||error)}`);
    showNotice(`CPU通話失敗：${compactFirebaseError(error?.message||error)}`);
    return false;
  }finally{cpuLiveStarting=false;updateCallUi()}
}
async function runLocalCpuCallReply(text){
  ...
  try{
    ...
    const prompt=JSON.stringify({
      task:'友達との個人通話として、相手の発言を具体的に拾い、自然で時々くすっとする返事をする',
...
async function autoStartMeetingVoice(){
  if(!meetingVoiceActive()||!canParticipateInMeeting())return false;
  const audioReady=await enableMeetingAudio(false);
  const micReady=await startMeetingVoice(true);
  meetingAutoVoicePending=!(audioReady&&micReady);
  if(audioReady&&micReady){setVoiceStatus('会議音声へ自動接続しました。マイクはONです。',true);return true}
  if(meetingVoiceActive())setVoiceStatus('自動接続できませんでした。「音声を聞く」または「マイク開始」を押してください。');
  return false;
}
...
function updateMeetingVoiceUi(){
  ...
  if(meetingVoiceStarting){button.textContent='🎙 自動接続中…';button.classList.remove('muted');return}
...
async function startMeetingVoice(auto=false){
  if(!meetingVoiceActive()||!canParticipateInMeeting())return false;
  if(meetingVoiceStream)return true;
  if(meetingVoiceStarting)return false;
  if(!navigator.mediaDevices?.getUserMedia&&!groupVoiceStream){setVoiceStatus('会議マイクにはHTTPSで開ける対応ブラウザが必要です。');if(!auto)showNotice('この環境ではマイクを使用できません。');return false}
  meetingVoiceStarting=true;updateMeetingVoiceUi();setVoiceStatus(auto?'会議音声へ自動接続しています…':'会議マイクの許可を待っています…');
  try{
    await enableMeetingAudio(false);
    meetingVoiceBorrowedFromGroup=Boolean(groupVoiceStream?.getAudioTracks?.().length);
    meetingVoiceStream=meetingVoiceBorrowedFromGroup?groupVoiceStream:await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,channelCount:1},video:false});
    meetingVoiceMuted=false;
    for(const track of meetingVoiceStream.getAudioTracks())track.enabled=true;
    const context=getVoiceAudioContext();
    if(!context)throw new Error('AudioContext unavailable');
    if(context.state==='suspended')await context.resume();
    meetingVoiceSource=context.createMediaStreamSource(meetingVoiceStream);
    meetingVoiceProcessor=context.createScriptProcessor(4096,1,1);
    ...
    meetingVoiceProcessor.onaudioprocess=event=>{
      ...
      for(let i=0;i<input.length;i+=4)energy+=input[i]*input[i];
      const rms=Math.sqrt(energy/Math.max(1,Math.ceil(input.length/4))),now=performance.now();
      if(rms>=.0035)meetingVoiceSpeechUntil=now+340;
      if(now>meetingVoiceSpeechUntil||now-meetingVoiceLastSentAt<68)return;
      meetingVoiceLastSentAt=now;
      const pcm=downsampleVoice(input,event.inputBuffer.sampleRate||context.sampleRate);if(!pcm.length)return;
      send('meetingVoiceAudio',{rate:RELAY_SAMPLE_RATE,seq:++meetingVoiceSequence,data:pcmToBase64(pcm)});
    };
    meetingAutoVoicePending=false;
    setVoiceStatus('会議音声へ自動接続しました。生存している全員に声が届きます。',true);return true;
  }catch(error){
    console.error('Meeting microphone error',error);
    stopMeetingVoice();
    setVoiceStatus('自動接続できませんでした。ブラウザのサイト設定でマイクを許可してください。');
    if(!auto)showNotice('会議マイクの使用が許可されませんでした。');
    return false;
...
function stopMeetingVoice(){
  ...
  if(!meetingVoiceBorrowedFromGroup){for(const track of meetingVoiceStream?.getTracks()||[])track.stop()}
  meetingVoiceStream=null;meetingVoiceStarting=false;meetingVoiceMuted=false;meetingVoiceBorrowedFromGroup=false;meetingVoiceSequence=0;meetingVoiceLastSentAt=0;meetingVoiceSpeechUntil=0;meetingAutoVoicePending=false;meetingVoicePlaybackAt.clear();
...
(function installHudLayoutFix(){
  ...
  style.textContent=`
    ...
    .cpu-controls{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin:8px 0 5px}
    .cpu-controls.hidden,.cpu-help.hidden{display:none!important}
    .cpu-controls button{min-width:0!important;padding:7px 8px!important;font-size:12px!important}
    .cpu-help{display:block;margin:0 2px 8px;color:#bfefff;opacity:.86;font-size:11px;line-height:1.4}
    .cpu-badge{flex:0 0 auto;padding:2px 5px;border-radius:999px;border:1px solid rgba(103,231,255,.42);background:rgba(40,172,213,.18);color:#9defff;font-size:9px;font-weight:900;letter-spacing:.04em}
    .player-row.cpu-player{border-color:rgba(88,220,255,.16)}
    .player-row.cpu-player .player-name{min-width:0}
